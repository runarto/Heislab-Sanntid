...
func (e *Elevator) DetermineMaster() {
	...
	for i, _ := range Elevators {
		if Elevators[i].isActive {
			masterCandidate = Elevators[i] // Set the first active elevator as the master candidate
			break
		}
	}

	for i, _ := range Elevators {
		fmt.Println("Elevator: ", Elevators[i].ID, "isActive: ", Elevators[i].isActive)
		if Elevators[i].isActive {
			if Elevators[i].ID < masterCandidate.ID { // If the elevator ID is less than the master candidate ID
				masterCandidate = Elevators[i] // Set the elevator as the master candidate
			}
		}
	}
